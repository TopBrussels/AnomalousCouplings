...
print "Looking at file : ", XMLFile.name
for xmlLine in XMLFile:
    xmlWord = xmlLine.split()
    if len(xmlWord) > 1 and xmlWord[0] == "13" and keepCounting == True:
        NrConfigs += 1
    ...
    if len(xmlWord) >= 2 and str(xmlWord[0]) == "nb_exp_events":
        NrEvts = int(xmlWord[1])
...
# Always make a new file such that naming convention is the same!
