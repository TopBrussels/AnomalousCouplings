...
c     Remove the (apriori) pointless permuation!
      ...
      write(*,*) 'After calling get_perm_weight in driver.f'
      write(*,*) 'Values of min_perm :',min_perm
...
c     re-init permvalue
      do perm_pos = 1, NPERM
         write(*,*) 'Before re-init:',perm_value(perm_pos,1),'(',
     &perm_pos,')'
...
c                See if this is require to continue to update this
     ...
     &                            '<', check_value
                ...
                if (montecarlo_perm) then
                    write(*,*) 'Value of cross : ',cross 
                    call check_nan(cross)
                    write(*,*) 'Value of cross after Nan check:',cross
                    if (cross.ne.0d0) then !CHANGED 6/02/15 (Mail Olivier)
                        !write(*,*) 'Perform all calls!'
                        call sort_perm
     ...
     &                                                       final_prec)
             ...
             write(*,*) 'cross', cross
             DO I =1,NPERM
                 write(*,*) 'After cross call:',perm_value(I,1),'-',I
                 perm_value(I,1) = 0d0
                 perm_error(I,1) = 0d0
             ENDDO
...
c                NCALL = NCALL * nb_sol_perm
 ...
 2           if (CROSS.lt.1d99.and.cross.ne.0d0) then
     ...
     &                  (cross + SD)* min_prec_cut1, check_value)
             ...
             DO J=1,NB_TF
                if (montecarlo_perm) then
                   ...
                   DO I=1,NPERM
                      write(*,*) 'Computing final value:',
     &perm_value_it(i,j),'(',i,',',j,')'
     ...
     &perm_value(i,j),'(',i,',',j,'('
                   ...
                   I = perm_pos
                   write(*,*) 'Computing final value (else):',
     &perm_value_it(i,j),'(',i,',',j,')'
                   value = perm_value_it(i,j)
                   error = perm_error_it(i,j)
                   call get_perm(I, loc_perm)
...
c     ==================================
      ...
      write(*,*) 'NDIM in new_perm_grid:',NDIM
      step = 1
      total = 0d0
      cross = 0d0
      DO I =1,NPERM
         write(*,*) 'Value_it in new_perm_grid:',perm_value_it(i,1),'-',i
...
c      write(*,*) '=>', cross
      ...
      do i=1,NPERM
           ...
           write(*,*) 'value in new grid :',perm_value_it(j,1),'-',j
...
c     First sort them
      do i=1,NPERM
         perm_order(i, config_pos) = i
      enddo
      write(*,*) 'data(1) in min_perm:',perm_value(1,1)
      data(1) = perm_value(1,1)
      do i=2,NPERM
         write(*,*) 'data(i) :',perm_value(i,1),'for',i
...
c     select them
      ...
      do i=1,NPERM
          j = perm_order(i, config_pos)
          tmp = perm_value(j,1)
          write(*,*) 'Value of perm_value =',tmp
          if (tmp/cross.lt.min_perm_cut) then
             write(*,*) 'This option should be excluded'
             min_perm(config_pos) = min_perm(config_pos) + 1
             write(*,*) 'New value of min_perm:',min_perm(config_pos)
