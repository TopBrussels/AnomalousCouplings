...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g u~ > b mu+ vm b~ s c~ u~
      PD(IPROC)=G1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g c~ > b mu+ vm b~ s c~ c~
      PD(IPROC)=G1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d~ > b mu+ vm b~ d u~ d~
      PD(IPROC)=G1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > b mu+ vm b~ d u~ s~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d~ > b mu+ vm b~ s c~ d~
      PD(IPROC)=G1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > b mu+ vm b~ s c~ s~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SMATRIX5(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        DSIG5=PD(0)*CONV*DSIGUU
      ELSE
