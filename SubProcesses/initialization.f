...
!Here you loop over all particles and sum the px and py components
!Hence at the end px_vis and py_vis are the negative of the neutrino kinematics
         tag_init(MG)=k
      enddo
     ...
     &           had_em(k) ,dummy1(k),dummy2(k)
      opt_lhco=k
      close(24)
 47   write(*,*) "read experimental data (pexp_init):"
      write(*,*) "-----------------------"
      write(*,*) " ET ", " Ex ", " Ey "," Ez "
!nexternal is the number of particles (with 1 and 2 being the two top quarks)
!num_inv is the number of neutrino particles
!Even with the name pexp the values should be energies!!
!This because the ET²-m² corresponds to the pT² value!
!Hence the rho variable in call_TF is the pT component!
      do j=3,nexternal-num_inv
!MG gives the particle number in the .lhco file (need -2 to be between 1 and 6)
         MG=inv_matching_type_part(j)
         write(*,*) (pexp_init(i,MG),i=0,3),'MG = ',MG
!The pexp_init values differ from the pt_init ones
!The latter one is the one actually inserted in the .lhco file!
!The former one is constructed from the px,py and pz components
!--> still need to find how these are obtained ...
      enddo
...
c
      variable(1)="theta"
      variable(2)="phi"
      variable(3)="pT"  !Changed on 10/02/15 from |p|
c
