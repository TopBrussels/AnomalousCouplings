...
if len(sys.argv) <= 1:
    ...
    print "Additional syntax can be : python LikCutInfluence_RecoEvts.py whichDir (opt - int/-1)CutValue (opt - y/n/-1)ExclOuterBins"
...
if len(sys.argv) > 2 and str(sys.argv[2]) != '-1':
    LikCuts = [int(sys.argv[2])]
    print "Will only look at cut-value = ", sys.argv[2]
if len(sys.argv) > 3 and str(sys.argv[3]) != '-1':
    ExclOuterBinsFit = [str(sys.argv[3])]
    print "Will exclude outer bins from fit: ", sys.argv[3]
            ...
            for File in os.listdir(whichDir):
                if File.startswith("FitDistributions") and File.endswith(".root"):
                    if ( (str(LikCuts[iCut]) != "100" and 'LikelihoodCut'+str(LikCuts[iCut]) in File) or (str(LikCuts[iCut]) == "100" and not 'LikelihoodCut' in File)) and ((ExclOuterBinsFit[iFit] == 'y' and 'OuterBinsExclForFit' in File) or (ExclOuterBinsFit[iFit] == 'n' and not 'OuterBinsExclForFit' in File)):
                        ROOTFile = whichDir + '' + File
                        if ExclOuterBinsFitStr[iFit] == "_OuterBinsExclForFit":
                        ...
                        if str(LikCuts[iCut]) == '100':
                            SaveHist.write('  hist_Sum->Draw(); \n  canv->SaveAs("SummedHist_DataEvts'+str(ExclOuterBinsFitStr[iFit])+'_NoLikCut.pdf"); \n\n')
                        else:
                            SaveHist.write('  hist_Sum->Draw(); \n  canv->SaveAs("SummedHist_DataEvts'+str(ExclOuterBinsFitStr[iFit])+'_LikCut'+str(LikCuts[iCut])+'.pdf"); \n\n')
                        SaveHist.write('}\n')
                        SaveHist.close()
...
# Output ROOT file
LatexOutput = open('LikCutInfluence_DataEvts.tex', 'w')
LatexOutput.write('\\documentclass[a4paper]{article} \n')
LatexOutput.write('\usepackage{graphicx} \n\usepackage{geometry}\n')
LatexOutput.write('\\begin{document} \n ')
LatexOutput.write(' \\begin{abstract} \n  Summary of the influence of the cut on the MadWeight likelihood distribution for data events.\n')
LatexOutput.write('  Will compare the difference in obtained minimum for data events. \n')
...
LatexOutput.write('   These distributions can be found in the following figure (Fig \\ref{fig::OrigResult} and \\ref{fig::OrigResult_AllBins}) for the data events. \n')
...
LatexOutput.write('    \\includegraphics[width = 0.75 \\textwidth]{SummedHist_DataEvts_OuterBinsExclForFit_NoLikCut.pdf} \n')
LatexOutput.write('    \\caption{MadWeight likelihood distributions (-ln($\\mathcal{L}$) for data events when the outer bins are excluded from the fit. This because the last bin lies rather far away from the expectations and is mainly added to avoid having an important value on the edge of the considered range.} \n')
LatexOutput.write('    \\label{fig::OrigResult} \n')
LatexOutput.write('   \\end{figure} \n \n')

LatexOutput.write('   \\begin{figure}[h!t] \n    \\centering \n')
LatexOutput.write('    \\includegraphics[width = 0.75 \\textwidth]{SummedHist_DataEvts_NoLikCut.pdf} \n')
LatexOutput.write('    \\caption{MadWeight likelihood distributions (-ln($\\mathcal{L}$) for data events when also the outer bins are used in the fit.} \n')
...
LatexOutput.write('   The MadWeight Likelihood distribution evaluated in the point $g_R$ = 0.0 for data events can be found in Figure \\ref{fig::SMLik}. \n')
LatexOutput.write('   \\begin{figure}[h!t] \n    \\centering \n')
LatexOutput.write('    \\includegraphics[width = 0.75 \\textwidth]{SMLikelihoodValue_DataEvts.pdf} \n')
...
for likCut in range(len(LikCuts)):
    if str(LikCuts[likCut]) != '100':
        ...
        LatexOutput.write('   \\begin{figure}[h!t] \n    \\centering \n')
        LatexOutput.write('    \\includegraphics[width = 0.75 \\textwidth]{SummedHist_DataEvts_OuterBinsExclForFit_LikCut' + str(LikCuts[likCut]) + '.pdf} \n')
        LatexOutput.write('    \\caption{MadWeight likelihood distributions (-ln($\\mathcal{L}$)) for data events after a likelihood cut of ' + str(LikCuts[likCut]) + ' (outer bins excluded from fit).} \n')
        LatexOutput.write('    \\label{fig::Cut' + str(LikCuts[likCut]) + '} \n')
        LatexOutput.write('   \\end{figure} \n \n')

        LatexOutput.write('   \\begin{figure}[h!t] \n    \\centering \n')
        LatexOutput.write('    \\includegraphics[width = 0.75 \\textwidth]{SummedHist_DataEvts_LikCut' + str(LikCuts[likCut]) + '.pdf} \n')
        LatexOutput.write('    \\caption{MadWeight likelihood distributions (-ln($\\mathcal{L}$)) for data events after a likelihood cut of ' + str(LikCuts[likCut]) + ' (outer bins used in  fit).} \n')
...
for LikCut in range(len(LikCuts)):
    if str(LikCuts[LikCut]) != '100':
        os.system('rm SummedHist_DataEvts_OuterBinsExclForFit_LikCut' + str(LikCuts[LikCut]) + '.pdf')
        os.system('rm SummedHist_DataEvts_LikCut' + str(LikCuts[LikCut]) + '.pdf')
