...
class TF_block(dict):
    ...
    def __init__(self,name):
        ...
        #init all to delta
        dict.__init__(self)
        self.update({'PT':TF_on_var(),'THETA':TF_on_var(),'PHI':TF_on_var()})
        self.order=0
        self.infotext=''
        self.name=name
...
class TF_in_SubProcesses:
    ...
    def text_get_central_point(self):
        ...
        for block in self.blockname_list:
            if isinstance(block, basestring):
                name_list='width_PT_'+block+', width_THETA_'+block+', width_PHI_'+block
            else:
                continue
        ...
        for i in range(0,len(self.blockname_list)):
            ...
            #define central point  (Changed on 10/02/15 rho to pt)
            if blockname != -1: #particle is visible
                ...
                text+=' c_point(perm, %s,3,1)=pt(pexp_init(0,2+perm_id(%s)))\n' %(i+1,i-1)
            
            #define width
            variable=['THETA','PHI','PT']
            for j in range(1,4):
    ...
    #2 #############################################################################     
    def text_tf_PT_for_one_part(self,i):
        """ return the different tf_PT_for_XX function in a  unformated text 
            (need to pass in mod_file for comment, and to f77_format    
        """  
