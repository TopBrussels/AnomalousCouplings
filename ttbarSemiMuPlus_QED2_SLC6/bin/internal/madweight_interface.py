...
#       here check validity of some parameters
    ...
    def submit_job(self, dirname, nb_card, sample_nb, evt_file=None, restrict_evt=[]):
        ...
        symlink=os.getenv('PWD')
        input_files = [pjoin(symlink, 'SubProcesses', dirname, 'comp_madweight'),
                       pjoin(symlink, 'Cards', 'param_card_%i.dat' % nb_card),
                       self.get_pdf_input_filename(),
                       pjoin(symlink, 'Cards', 'ident_card.dat'),
                       pjoin(symlink, 'Cards', 'run_card.dat')
                       ]
        ...
        input_files.append(pjoin(symlink, 'SubProcesses', dirname, name, evt))
        
        if restrict_evt:
            restrict_path = pjoin(symlink, 'SubProcesses', dirname, name,
        ...
        exe = pjoin(symlink, 'bin', 'internal', 'madweight', 'MW_driver.py')
        ...
        cwd = pjoin(symlink, 'SubProcesses', dirname, name)
        ...
        if self.options['run_mode'] == 0:
            ...
            self.cluster.temp_dir=pjoin(symlink, 'Events')
